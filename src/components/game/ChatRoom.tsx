...
export function ChatRoom({ channel = 'global' }: { channel?: string }) {
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`font-mono-game text-[9px] uppercase px-2 py-0.5 border transition-colors ${
              showLeaderboard ? 'border-accent bg-accent/10 text-accent' : 'border-border text-muted-foreground hover:text-foreground'
            }`}
          >
            {showLeaderboard ? 'Chat' : 'Leaderboard'}
          </button>
      ...
      {showLeaderboard ? (
        <div className="flex-1 overflow-y-auto p-3 min-h-0" style={{ maxHeight: '50vh' }}>
          <Leaderboard />
        </div>
      ) : (
        <>
